...
<article id="bo_v" class="container">
    ...
    <section id="bo_v_info">
        <h2>페이지 정보</h2>
        <strong><span class="sv_guest">${programVO.id }</span></strong>
        <span class="sound_only"></span><strong><fmt:formatDate pattern="yyyy-MM-dd HH:mm:ss"
							value="${programVO.regidate}" /></strong>
        조회수<strong>${programVO.viewcnt }</strong>
    </section>
    ...
    <div id="bo_v_top">
       ...
       <c:if test="${login.user_id ne programVO.id }"> 
       		<ul class="bo_v_com">
    ...
    <section id="bo_v_atc" class="col-md-10 col-md-offset-1">
        ...
        <!-- 본문 내용 시작 { -->
        <div id="bo_v_con">${programVO.contents }</div>
         <!-- } 본문 내용 끝 -->
    </section>

<!-- 지원자 시작 { -->
<c:if test="${login.user_id eq programVO.id }">
<section id="bo_vc" class="clearfix">
    <div class="col-md-8 col-md-offset-2">
		...
		<c:forEach items="${rlist}" var="resumeVO">
			...
			<tr>
				<c:if test="${resumeVO.type eq 'p' and resumeVO.idx eq programVO.idx and resumeVO.accept ne 1}">
					<td>${resumeVO.name} <input type="hidden" name="uno" value="${resumeVO.uno }"/><a href="/program/resumeRead?uno=${resumeVO.uno }&idx=${programVO.idx}&sid=${programVO.id}&title=${programVO.title}" style="color:black;" onclick="window.open(this.href, '팝업창', 'width=500, height=500'); return false;"><button class="btn btn-sm btn-primary">이력서 보기</button></a></td>
				</c:if>
...
function popupOpen() {
	...
	var popUrl = "/program/resume?idx="+ ${programVO.idx} +"&type=p";	//팝업창에 출력될 페이지 URL
...
<script src="http://witch02.dothome.co.kr/01/js/md5.js"></script>
    ...
    <div id="bo_v_bot">
        <ul class="bo_v_com">
        <c:if test="${login.user_id eq programVO.id }">
            <li><a href="/program/modify?idx=${programVO.idx }" class="btn btn-sm btn-primary">수정</a></li>
            <li><a href="/program/remove?idx=${programVO.idx }" class="btn btn-sm btn-primary">삭제</a></li>
        </c:if>
            <li><a href="/program/listAll" class="btn btn-sm btn-success">목록</a></li>
        </ul>
   </div>
    <!-- } 링크 버튼 끝 -->
    </div>
