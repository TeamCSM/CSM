		...
		$.ajax({
			...
			success : function(d){
				...
				contents +=	"<tbody id='list'>";
				 $.each(d.list, function(index, CustomerVO){
					contents +="<tr>";
					contents +="<td>"+CustomerVO.notice_bno+"</td>";
					contents +="<td><a href='../sQna/readPage${pageMaker.makeSearch(pageMaker.cri.page)}&bno='"+CustomerVO.notice_bno+"'>"+CustomerVO.notice_title+"</a></td>";
					contents +="<td>"+CustomerVO.notice_writer+"</td>";
					contents +="<td>"+regidate+"</td>";
					contents +="<td><span class='badge bg-red'>"+CustomerVO.notice_viewcnt+"</span></td>";
					contents +="</tr>";	
				}); 
				contents +="</table>";
	...
	<div class="tbl_head01 tbl_wrap table-responsive">
        ...
        <tbody id="list">
			...
			<c:forEach items="${list}" var="CustomerVO">
				<tr align="center">
					<td>${CustomerVO.notice_bno}</td>
					<td><a href='../sNotice/readPage${pageMaker.makeSearch(pageMaker.cri.page)}&bno=${CustomerVO.notice_bno}'>${CustomerVO.notice_title}</a></td>
					<td>${CustomerVO.notice_writer}</td>
					<td><fmt:formatDate pattern="yyyy-MM-dd HH:mm"
							value="${CustomerVO.notice_regidate}" /></td>
					<td><span class="badge bg-red">${CustomerVO.notice_viewcnt}</span></td>
				</tr>
